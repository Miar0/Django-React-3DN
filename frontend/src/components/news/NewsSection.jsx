    ...
    return (
        <section className="relative  w-full py-8 px-4 lg:px-16 text-[#1B1B1B] dark:text-white bg-[--color-news-bg-light] dark:bg-transparent">
            ...
            <div
                ...
                {/* Новини */}
                <div className="w-full lg:w-[703px] flex flex-col gap-4">
                    {news.map((item) => (
                        <Link
                            to={`/news/${item.id}`}
                            key={item.id}
                            className="group block w-full lg:w-[703px] h-auto lg:h-[168px] relative rounded-xl overflow-hidden
            ...
            group-hover:before:opacity-100"
                        >
                            <div
                                className="flex flex-col lg:flex-row gap-4 w-full h-full p-4">
                                <div
                                    className="w-full lg:w-[168px] h-[200px] lg:h-full overflow-hidden rounded-lg">
                                    <img
                                        src={item.image}
                                        alt={item.title}
                ...
                {/* Зображення справа */}
                <div className="hidden lg:flex flex-1 justify-end items-start mr-2">
                    <div
                        className="w-full max-w-[444px] h-full overflow-hidden rounded-2xl group">
                        <img
                            src={petsImage}
                            alt="Pets"
                            className="w-full h-full object-cover rounded-2xl shadow-xl transition-transform duration-500 ease-in-out group-hover:scale-105"
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-center mt-10">
                <Link to="/news" className="group">
                    ...
                    dark:border-white dark:text-white dark:bg-transparent dark:hover:bg-white/10 cursor-pointer">
                        Дізнатись більше
                        <span
                            className="transition-transform duration-300 group-hover:translate-x-1">➜</span>
                    </button>
                </Link>
            </div>
        </section>
